...
impl Editor {
    ...
    pub fn stage_diff_hunks(&mut self, ranges: Vec<Range<Anchor>>, cx: &mut Context<Self>) {
        ...
        for (buffer_id, hunks) in &chunk_by {
            ...
            let mut base = secondary_diff.base_text().map_or_else(
                || Rope::from(""),
                |snapshot| snapshot.text.as_rope().clone(),
            );
            log::debug!("original: {:?}", base.to_string());
            for hunk in hunks {
                ...
                base.replace(hunk.diff_base_byte_range.clone(), &replacement_text);
                log::debug!("replace with {replacement_text:?}");
            }
            log::debug!("final base: {:?}", base.to_string());

            project
                .git_state()
                .read(cx)
                .set_index_text(git_repo, path, base);
        }
    }
